...
from MasterThesis_Config import NN_COMBINED_DATA_FOLDER, NN_CLASSIFICATION_FILE, LOGS_FOLDER, CLASSIFICATION_FOLDER
...
def create_training_validation_sets(data_file, classification_file, segment_info_file, log_file_path):
    ...
    # Stratified K-Fold based on unique child IDs
...
class IMUBigDataset(Dataset):
    def __init__(self, data_file, classification_file):
        ...
        self.lengths = [6000] * len(self.labels)  # Updated to reflect 60-second segments
...
class IMULSTM(nn.Module):
    def __init__(self, input_size=24, hidden_size=512, num_layers=4, num_classes=2, dropout=0.6):
...
def train_lstm_on_bigdata(data_file, classification_file, segment_info_file, log_file_path):
    ...
    for train_segments, train_labels, val_segments, val_labels in fold_sets:
        ...
        # Adjusting class weights to emphasize FM- detection
        class_counts = np.bincount(train_labels)
        weight_for_class_0 = max(class_counts) / class_counts[0] * 3.0  # Increase weight for the minority class
        weight_for_class_1 = 1.0
        class_weights = torch.tensor([weight_for_class_0, weight_for_class_1], dtype=torch.float)

        # DataLoaders with adjusted weights and sampling strategy
        train_loader = DataLoader(train_dataset, batch_size=128, shuffle=True, num_workers=4, persistent_workers=True)
        val_loader = DataLoader(val_dataset, batch_size=256, shuffle=False, num_workers=4, persistent_workers=True)
        ...
        with torch.no_grad():
            for idx, (inputs, labels) in enumerate(val_loader):
                ...
                predicted = (probabilities[:, 1] > 0.3).long()  # Lowering threshold to 0.3
...
if __name__ == "__main__":
    ...
    # Update paths to match the new folder
    data_file = os.path.join(NN_COMBINED_DATA_FOLDER, 'IMU_Segmented_60_Seconds', 'IMU_Segmented_60s_BigDataset.npy')
    classification_file = os.path.join(NN_COMBINED_DATA_FOLDER, 'IMU_Segmented_60_Seconds', 'FM_QualityClassification_Binary_60s_BigData.csv')
    segment_info_file = os.path.join(NN_COMBINED_DATA_FOLDER, 'IMU_Segmented_60_Seconds', 'IMU_Segmented_60s_Group_Info.csv')
    log_file_path = os.path.join(LOGS_FOLDER, 'LSTM_60s_BigData_Log.txt')
