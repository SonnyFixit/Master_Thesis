...
def create_training_validation_sets(data_file, classification_file, segment_info_file, log_file_path):
    ...
    for train_idx, test_idx in skf.split(unique_ids, labels):
        ...
        for child_id in train_ids:
            ...
            valid_segments = child_data['Segment ID'].values - 1  # Adjusting for 0-based indexing
...
# Dataset class for Kinect data
class KinectBigDataset(Dataset):
    def __init__(self, data_file, classification_file):
        ...
        self.lengths = [99] * len(self.labels)  # Each sample has 99 timesteps (total points * 3 coordinates)
...
# Definition of the LSTM model adapted for Kinect data
class KinectLSTM(nn.Module):
    def __init__(self, input_size=99, hidden_size=512, num_layers=4, num_classes=2, dropout=0.6):
        super(KinectLSTM, self).__init__()
...
def train_lstm_on_kinect_data(data_file, classification_file, segment_info_file, log_file_path):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    dataset = KinectBigDataset(data_file, classification_file)
    ...
    for train_segments, train_labels, val_segments, val_labels in fold_sets:
        ...
        # Adjust class weights with emphasis on negative samples
        class_counts = np.bincount(train_labels)
        weight_for_class_0 = max(class_counts) / class_counts[0] * 4.0  # Higher weight for negative class
        ...
        model = KinectLSTM(input_size=99, hidden_size=256, num_layers=3, num_classes=2, dropout=0.75).to(device)
        ...
        for epoch in range(num_epochs):
            ...
            for i, (inputs, labels) in enumerate(train_loader):
                ...
                optimizer.zero_grad()
                outputs = model(inputs, torch.tensor([inputs.size(1)] * inputs.size(0), device=device))
...
if __name__ == "__main__":
    ...
    data_file = "/content/drive/My Drive/Kinect_Combined_BigDataset.npy"
    classification_file = "/content/drive/My Drive/Kinect_QualityClassification_Binary_BigData.csv"
    segment_info_file = "/content/drive/My Drive/Kinect_Segmented_Group_Info.csv"
    log_file_path = "/content/drive/My Drive/Kinect_BigData_Log.txt"

    train_lstm_on_kinect_data(data_file, classification_file, segment_info_file, log_file_path)
