...
# Define a consistent color palette for LSTM-CNN (darker green color scheme)
color_palette = sns.light_palette("forestgreen", as_cmap=True)
...
table[0, 0].set_facecolor('#A9A9A9')  # Slightly darker gray for headers
for key, cell in table.get_celld().items():
    if key[0] == 0 or key[1] == 0:
        cell.set_facecolor('#A9A9A9')  # Consistent gray color for headers
