...
from MasterThesis_Config import SEGMENTED_IMU_DATA_FRAMES_FOLDER, COMBINED_DATA_FOLDER, LOGS_FOLDER, CLASSIFICATION_FOLDER, BINARY_CLASSIFICATION_FILE_CSV

# Define new folder for 60-second segments
SEGMENTED_60S_FOLDER = os.path.join(COMBINED_DATA_FOLDER, 'IMU_Segmented_60_Seconds')
...
BIG_DATASET_FILE = os.path.join(SEGMENTED_60S_FOLDER, 'IMU_Segmented_60s_BigDataset.npy')
LOG_FILE_PATH = os.path.join(SEGMENTED_60S_FOLDER, 'IMU_Segmented_60s_BigDataset_Log.txt')

# Update the output path for the classification CSV file
CLASSIFICATION_OUTPUT_FILE = os.path.join(SEGMENTED_60S_FOLDER, 'FM_QualityClassification_Binary_60s_BigData.csv')

# New output CSV file for ID, group, and range information
SEGMENT_INFO_OUTPUT_FILE = os.path.join(SEGMENTED_60S_FOLDER, 'IMU_Segmented_60s_Group_Info.csv')
...
def create_big_dataset_60s(combined_folder=COMBINED_DATA_FOLDER, output_file=BIG_DATASET_FILE):
    ...
    for file_name in sorted(os.listdir(combined_folder)):
        if file_name.startswith("IMU_CombineSegmentedData_") and file_name.endswith(".npy"):
            ...
            try:
                ...
                # Add information about the child and new segments
                for segment_index in range(num_full_segments):
                    segment_id = f"{total_segments + 1}"  # Sequential ID starting from 1
                    child_data_info.append((segment_id, child_id, valid_data.shape))
                    total_segments += 1
    ...
    for segment_id, child_id, shape in child_data_info:
        log_message(f"Child ID: {child_id}, Segment ID: {segment_id}, Data Shape: {shape}")
...
def create_classification_file_60s(child_data_info, classification_file_path=BINARY_CLASSIFICATION_FILE_CSV, output_file_path=CLASSIFICATION_OUTPUT_FILE, excluded_samples=[]):
    ...
    for segment_id, child_id, shape in child_data_info:
        ...
        classification_data.append([segment_id, classification]) 

        # Log information about the current child data
        log_message(f"Segment ID: {segment_id} - Assigned classification '{classification}'.")
    ...
    with open(output_file_path, 'w', newline='') as csvfile:
        ...
        csv_writer.writerow(["Segment ID", "Classification"])  # Write header
